...
$sql = "select id_spek, data, tytul, cena, nr_sali, gatunek, teatr_FK from spektakl s, widz w where w.ulubiony_gatunek = s.gatunek and w.PESEL = :pes";
...
if(!$r) {
    $txt = "Blad przy znalezienia spektakli.";
    print "<h2>" . $txt . "</h2>";
...
while ($row1 = oci_fetch_array($compiled, OCI_ASSOC+OCI_RETURN_NULLS)) {
    ...
    echo '    <td><a href="buy_ticket_fav.php?nr=';
    echo $ord;
    echo '&spek=' . $row1["ID_SPEK"];
    echo '&klient=' . $pes;
    echo '&cena=' .$row1["CENA"];
    echo '">Kup</a></td>';
    echo '    <td><a href="cast_fav.php?nr=';
    echo $ord;
    echo '&spek=' . $row1["ID_SPEK"];
